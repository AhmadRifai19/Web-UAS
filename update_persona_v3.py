...
for panel_idx, data in personas.items():
    ...
    # Replace each sticky section - ONLY replace the sticky items between <div class="persona__stickies"> and </div>
    for title_class, title_text, data_key in [
        ...
        title_pos = panel_html.find(f'persona__section-title--{title_class}">{title_text}</h4>')
        if title_pos == -1:
            print(f"Panel {panel_idx}: {title_text} NOT FOUND")
            continue
        
        # Find the stickies container after this title
        stickies_open = panel_html.find('<div class="persona__stickies">', title_pos)
        if stickies_open == -1:
            print(f"Panel {panel_idx}: {title_text} stickies container NOT FOUND")
            continue
        
        # Find the closing </div> of the stickies container using depth tracking
        depth = 0
        scan_start = stickies_open
        stickies_close = -1
        for m in re.finditer(r'<div[\s>]|</div>', panel_html[scan_start:]):
            if m.group().startswith('<div'):
                depth += 1
            else:
                depth -= 1
            if depth == 0:
                stickies_close = scan_start + m.start()
                break
        
        if stickies_close == -1:
            print(f"Panel {panel_idx}: {title_text} stickies close NOT FOUND")
            continue
        
        # Replace only the content BETWEEN <div class="persona__stickies"> and </div>
        content_start = stickies_open + len('<div class="persona__stickies">')
        new_stickies = '\n' + make_stickies(data[data_key]) + '\n'
        panel_html = panel_html[:content_start] + new_stickies + panel_html[stickies_close:]
        changes += 1
    
    # Replace Short Description (uses <p> tag)
    sd_start = panel_html.find('<p class="persona__desc-text">')
    sd_end = panel_html.find('</p>', sd_start)
    if sd_start != -1 and sd_end != -1:
        sd_content_start = sd_start + len('<p class="persona__desc-text">')
        panel_html = panel_html[:sd_content_start] + data["short_description"] + panel_html[sd_end:]
        changes += 1
    else:
        print(f"Panel {panel_idx}: Short Description NOT FOUND")
    
    # Replace the panel content back
    # Recalculate panel_end since panel_html length changed
    if panel_idx < 4:
        panel_end = content.find(f'data-panel="{panel_idx + 1}"', panel_start + 1)
    else:
        panel_end = content.find('<!-- Define Problem Statement', panel_start)
