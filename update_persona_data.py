...
personas = {
    ...
    1: {  # Susanto Hari Wibowo (Frame 112)
        "key_attribute": [
    ...
    2: {  # Haris Ridho Ramadhan (Frame 113)
        "key_attribute": [
            "Mahasiswa aktif.",
    ...
    3: {  # Widya Dny Yuanika R. (Frame 110)
        "key_attribute": [
...
for panel_idx, data in personas.items():
    ...
    # 1. Replace Key Attribute stickies
    ka_pattern = re.compile(
        r'(<h4 class="persona__section-title persona__section-title--blue">Key Attribute</h4>\s*'
        r'<div class="persona__stickies">)\s*'
        r'(.*?)'
        r'\s*(</div>\s*</div>)',
        re.DOTALL
    )
    ka_match = ka_pattern.search(panel_html)
    if ka_match:
        new_stickies = make_stickies(data["key_attribute"])
        new_ka = ka_match.group(1) + '\n' + new_stickies + '\n' + ka_match.group(3)
        panel_html = panel_html[:ka_match.start()] + new_ka + panel_html[ka_match.end():]
        changes += 1
    else:
        print(f"Panel {panel_idx}: Key Attribute NOT FOUND")
    
    # 2. Replace Short Description
    ...
    if sd_match:
        new_sd = sd_match.group(1) + data["short_description"] + sd_match.group(2)
        panel_html = panel_html[:sd_match.start()] + new_sd + panel_html[sd_match.end():]
        changes += 1
    else:
        print(f"Panel {panel_idx}: Short Description NOT FOUND")
    
    # 3. Replace Needs stickies
    needs_pattern = re.compile(
        r'(<h4 class="persona__section-title persona__section-title--blue">Needs</h4>\s*'
        r'<div class="persona__stickies">)\s*'
        r'(.*?)'
        r'\s*(</div>\s*</div>)',
        re.DOTALL
    )
    needs_match = needs_pattern.search(panel_html)
    if needs_match:
        new_needs = make_stickies(data["needs"])
        new_n = needs_match.group(1) + '\n' + new_needs + '\n' + needs_match.group(3)
        panel_html = panel_html[:needs_match.start()] + new_n + panel_html[needs_match.end():]
        changes += 1
    else:
        print(f"Panel {panel_idx}: Needs NOT FOUND")
    
    # 4. Replace Challenges stickies
    ch_pattern = re.compile(
        r'(<h4 class="persona__section-title persona__section-title--blue">Challenges</h4>\s*'
        r'<div class="persona__stickies">)\s*'
        r'(.*?)'
        r'\s*(</div>\s*</div>)',
        re.DOTALL
    )
    ch_match = ch_pattern.search(panel_html)
    if ch_match:
        new_ch = make_stickies(data["challenges"])
        new_c = ch_match.group(1) + '\n' + new_ch + '\n' + ch_match.group(3)
        panel_html = panel_html[:ch_match.start()] + new_c + panel_html[ch_match.end():]
        changes += 1
    else:
        print(f"Panel {panel_idx}: Challenges NOT FOUND")
    
    # 5. Replace Opportunities stickies
    opp_pattern = re.compile(
        r'(<h4 class="persona__section-title persona__section-title--orange">Opportunities</h4>\s*'
        r'<div class="persona__stickies">)\s*'
        r'(.*?)'
        r'\s*(</div>\s*</div>)',
        re.DOTALL
    )
    opp_match = opp_pattern.search(panel_html)
    if opp_match:
        new_opp = make_stickies(data["opportunities"])
        new_o = opp_match.group(1) + '\n' + new_opp + '\n' + opp_match.group(3)
        panel_html = panel_html[:opp_match.start()] + new_o + panel_html[opp_match.end():]
        changes += 1
    else:
        print(f"Panel {panel_idx}: Opportunities NOT FOUND")
    
    # Replace the panel content back
    content = content[:panel_start] + panel_html + content[panel_end:]
    print(f"Panel {panel_idx}: Updated all 5 sections")
